...
def percentile_value(sorted_vals: list, pct: float) -> float:
    """Value at the given percentile of an already-sorted list of scores."""
    if not sorted_vals:
        return 0.0
...
def collect_class_scores(src: Path, cls_name: str, has_splits: bool) -> list:
    """
    Score every image for one class, pooling across all splits (train/val/test)
    if the dataset is split-layout, so the derived threshold is one consistent
    cutoff applied everywhere rather than a different one per split.
    """
    exts = {".jpg", ".jpeg", ".png"}
    dirs = []
    if has_splits:
    ...
    for d in dirs:
        for f in d.iterdir():
            if f.suffix.lower() in exts:
                try:
                    scores.append(plant_score(str(f)))
                except Exception:
                    pass
    return sorted(scores)


def derive_percentile_thresholds(src: Path, has_splits: bool, pct_map: dict) -> dict:
    """
    pct_map: {cls_name: (bad_pct, small_pct)} percentile RANKS (0-100).
    Returns {cls_name: (bad_thresh, small_thresh)} actual plant_score fractions,
    read off this dataset's own distribution instead of a hardcoded guess.
    """
    print(f"\n[percentile mode] Deriving thresholds from this dataset's own distribution...")
    thresholds = {}
    for cls_name, (bad_pct, small_pct) in pct_map.items():
        scores = collect_class_scores(src, cls_name, has_splits)
        if not scores:
            print(f"  {cls_name}: no images found — skipping")
            continue
        bad_val = percentile_value(scores, bad_pct)
        small_val = percentile_value(scores, small_pct)
        thresholds[cls_name] = (bad_val, small_val)
        print(f"  {cls_name}: {bad_pct:.0f}th percentile = {bad_val*100:.2f}% (drop cutoff), "
              f"{small_pct:.0f}th percentile = {small_val*100:.2f}% (zoom cutoff)  "
              f"[{len(scores)} images pooled across all splits]")
    return thresholds
...
def main():
    ...
    # Percentile-based thresholds — derive bad/small cutoffs from this dataset's own
    # distribution instead of the fixed fractions above, for datasets where the
    # hardcoded absolute cutoffs don't fit (e.g. an overall darker/sparser batch).
    ap.add_argument("--use_percentiles", action="store_true",
                    help="Derive bad/small thresholds from this dataset's own percentile "
                         "distribution instead of the fixed --*_bad/--*_small fractions above")
    ap.add_argument("--cn_coty_bad_pct",   type=float, default=5,
                    help="cn_coty:  percentile rank to use as the drop cutoff (default 5th)")
    ap.add_argument("--cn_coty_small_pct", type=float, default=25,
                    help="cn_coty:  percentile rank to use as the zoom cutoff (default 25th)")
    ap.add_argument("--non_coty_bad_pct",  type=float, default=5,
                    help="non_coty: percentile rank to use as the drop cutoff (default 5th)")
    ap.add_argument("--non_coty_small_pct",type=float, default=25,
                    help="non_coty: percentile rank to use as the zoom cutoff (default 25th)")
    ...
    if args.use_percentiles:
        pct_map = {
            "cn_coty":  (args.cn_coty_bad_pct,  args.cn_coty_small_pct),
            "non_coty": (args.non_coty_bad_pct, args.non_coty_small_pct),
        }
        derived = derive_percentile_thresholds(src, has_splits, pct_map)
        class_thresholds.update(derived)   # overrides the fixed-fraction defaults above

    print(f"\n{'='*60}")
    print(f"Source : {src}")
    print(f"Layout : {'train/val splits' if has_splits else 'flat class dirs'}")
    print(f"Threshold mode: {'percentile-derived' if args.use_percentiles else 'fixed fraction'}")
    ct_bad, ct_small = class_thresholds["cn_coty"]
    nt_bad, nt_small = class_thresholds["non_coty"]
    print(f"cn_coty  thresholds: bad<{ct_bad*100:.2f}%  zoom<{ct_small*100:.2f}%")
    print(f"non_coty thresholds: bad<{nt_bad*100:.2f}%  zoom<{nt_small*100:.2f}%")
